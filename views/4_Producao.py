...
def _renderizar_resultado(dados: dict):
    ...
    for pos, (procedimento, qtd) in enumerate(dados["ranking"], 1):
        pct = (qtd / dados["total_linhas"]) * 100
        valor = dados["valores"].get(procedimento, 0.0)
        linhas_resumo.append(f"{pos}. {procedimento} - {qtd} proc(s) ({_fmt_pct(pct)}) | Vl. pago somado: {_fmt_moeda(valor)}")
...
if uploaded_files:
    ...
    if "_producao_dados_pdf" in st.session_state:
        _renderizar_resultado(st.session_state["_producao_dados_pdf"])
