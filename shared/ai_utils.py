...
def gerar_texto_ocorrencia_com_ia(texto: str) -> tuple[str, str]:
    ...
    (essa continua sendo melhorar_texto_com_ia, sobre o texto de orientação).

    Ao contrário de melhorar_texto_com_ia, esta função PODE receber número de
    guia (decisão consciente: risco considerado baixo, guia sozinho sem nome
    de prestador/paciente/CPF). Mesma ressalva de sempre sobre a camada
    gratuita do Gemini valendo pro resto (não mandar nome de prestador/CPF
    aqui também).

    Retorna (texto_resultado, erro) no mesmo formato de melhorar_texto_com_ia.
    """
